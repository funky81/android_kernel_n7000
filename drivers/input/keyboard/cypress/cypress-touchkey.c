...
static int i2c_touchkey_read(u8 reg, u8 *val, unsigned int len)
{
	...
	if ((touchkey_driver == NULL) || !(touchkey_enable == 1)
	    || !touchkey_probe) {
		print_debug(__func__,"!enable !probe");
		if (!touchkey_probe)
			print_debug(__func__,"!probe");
		return -ENODEV;
	}
...
static int sec_touchkey_late_resume(struct early_suspend *h)
{
	...
	if (BLN_ongoing) {
		...
		if (wake_lock_active(&bln_wake_lock)) {
			print_debug(__func__,"try to clear wakelock");
...
|| defined(CONFIG_TARGET_LOCALE_NA) || defined(CONFIG_MACH_Q1_BD)
	...
	if (touchled_cmd_reversed) {
		...
		print_debug(__func__,"LED Should Resume now");
...
/* guard */
	...
	if (led_timeout > 0) {
		print_debug(__func__,"Restart Timer");
		mod_timer(&led_timer, jiffies + msecs_to_jiffies(led_timeout));
	}
...
static void bln_late_resume(struct early_suspend *h) {
	...
	if (touchkey_enable == 1) {
		...
		/* we were using a wakelock, unlock it */
		if (wake_lock_active(&bln_wake_lock)) {
			print_debug(__func__,"clear wakelock");
			wake_unlock(&bln_wake_lock);
		}
	...
	touchkey_activate();
	msleep(50);
	print_debug(__func__,"should be on right now");
}
...
static void enable_touchkey_backlights(void) {
	print_debug(__func__,"enable LED from BLN app");
	int status = 0x10;
	i2c_touchkey_write((u8 *)&status, 1);
	msleep(50);
}
...
static void disable_touchkey_backlights(void) {
	print_debug(__func__,"disable LED from BLN app");
	int status = 0x20;
	i2c_touchkey_write((u8 *)&status, 1);
	msleep(50);
}
...
static void enable_led_notification(void) {
	...
	if (bln_enabled) {
		...
		if (touchkey_enable == 1) {
			print_debug(__func__,"BLN ongoing set to true");
			BLN_ongoing = true;
			enable_touchkey_backlights();
		}
		/* See if a timeout value has been set for the notification */
		if (notification_timeout > 0) {
			print_debug(__func__,"timeout > 0");
			/* restart the timer */
...
static void disable_led_notification(void) {
	...
	BLN_ongoing = false;
	print_debug(__func__,"BLN ongoing set to false");

	if (touchkey_enable == 1) {
		print_debug(__func__,"enable==1");
		disable_touchkey_backlights();
		if (bln_suspended) {
			print_debug(__func__,"bln==1 suspended");
			touchkey_deactivate();
		}
	...
	if (sscanf(buf, "%u\n", &data) == 1) {
		if (data == 0 || data == 1) {
			...
			if (data == 0) {
				...
				bln_enabled = false;
				if (BLN_ongoing){
					print_debug(__func__,"bln ongoing 1");
					disable_led_notification();
				}
				if (wake_lock_active(&bln_wake_lock)) {
					print_debug(__func__,"Wake lock active 1");
					wake_unlock(&bln_wake_lock);
				}
			}
		} else {
			/* error */
			print_debug(__func__,"masuk error");
		}
	} else {
		if (!strncmp(buf, "on", 2)){
			bln_enabled = true;
			print_debug(__func__,"data=on");
		}
		if (!strncmp(buf, "off", 3)) {
			print_debug(__func__,"data=off");
			bln_enabled = false;
			if (BLN_ongoing){
				print_debug(__func__,"bln ongoing 2");
				disable_led_notification();
			}
			if (wake_lock_active(&bln_wake_lock)) {
				print_debug(__func__,"Wake lock active 2");
				wake_unlock(&bln_wake_lock);
			}
		}
	}
        ...
        if (sscanf(buf, "%u\n", &data) == 1) {
            if (data == 0 || data == 1) {
                ...
                if (data == 0){
                    ...
                    disable_led_notification();
    				if (wake_lock_active(&bln_wake_lock)) {
    					print_debug(__func__,"data==0 & disable wake lock");
    					wake_unlock(&bln_wake_lock);
    				}
                }
            } else {
...
{
	printk(KERN_ERR "[TouchKey]  Breathe");
	int data;
	if (BLN_ongoing != 1 || touchkey_enable != 1)
		return;
	printk(KERN_ERR "[TouchKey]  Breathe Steps");
			      ...
			      const struct i2c_device_id *id)
{
	...
	print_debug(__func__,"i2c probe");
...
|| defined(CONFIG_TARGET_LOCALE_NA) || defined(CONFIG_MACH_Q1_BD)
	...
	if (device_create_file(sec_touchkey,&dev_attr_led_timeout) < 0) {
